...
class plateau(val longueur : Int, val largeur : Int, val x1: Float, val y1: Float, val x2: Float, val y2: Float, joueur1 : Joueur, joueur2 : Joueur){
    ...
    fun creer_liste_de_case(): MutableList<MutableList<case>>{
        ...
        for (i in 0..largeur-1){
            liste_de_case[i][1].occupant = pion("pion${liste_de_case[i][1].absysse}", liste_de_case[i][1],"blanc", true,joueur1)
            liste_de_case[i][6].occupant = pion("pion${liste_de_case[i][6].absysse}", liste_de_case[i][6],"noir", true, joueur2)
            if (i==1 || i==6){
